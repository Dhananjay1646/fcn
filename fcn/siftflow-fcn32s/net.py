...
def fcn(split):
    ...
    n.data, n.sem, n.geo = L.Python(module='siftflow_layers',
            ...
            param_str=str(dict(siftflow_dir='/home/tramac/caffe/examples/fcn/data/sift-flow',
    ...
    n.score_fr_sem = L.Convolution(n.drop7, num_output=2, kernel_size=1, pad=0, # 33->2
        param=[dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)])
    n.upscore_sem = L.Deconvolution(n.score_fr_sem,
        convolution_param=dict(num_output=2, kernel_size=64, stride=32,  ##change 33->2
            bias_term=False),
    ...
    n.score_fr_geo = L.Convolution(n.drop7, num_output=2, kernel_size=1, pad=0, #3->2
        param=[dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)])
    n.upscore_geo = L.Deconvolution(n.score_fr_geo,
        convolution_param=dict(num_output=2, kernel_size=64, stride=32, # 3->2
            bias_term=False),
        param=[dict(lr_mult=0)])
